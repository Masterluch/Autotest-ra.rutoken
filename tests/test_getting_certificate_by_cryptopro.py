...
def test_getting_certificate_by_cryptopro(get_browser, get_algorithms_list):
    ...
    for i in range(len(list_algorithms)):
        list_certificates.append(Certificate(
            browser, f"{browser.browser_name}Key{i}", list_algorithms[i], is_test_mode=False))
        logger.debug(
            f"Создали сертификат для ключа {browser.browser_name}Key{i}")

    # Проверяем запросы
    for cert in list_certificates:
        if not (cert.is_created):
            logger.error(
                f"Поле is_created сертификата для {browser.browser_name}Key{i} = False")
            assert(False)
