...
class Signature:
    def __init__(self, browser, key_id: str, algorithm: str, is_attached_mode: bool = True):
        ...
        browser.click_on_element_by(
            By.XPATH, f"//*[contains(text(), '{key_id}')]/../../../..//button[contains(text(), 'Подписать')]")

        # Выбираем, в каком виде хотим видеть результат
        if (is_attached_mode):
            # Выбираем вариант 'Подпись объединена с документом'
            browser.click_on_element_by(
                By.XPATH, "//*[contains(text(), 'Подпись объединена с документом')]")
        else:
            # Выбираем вариант 'Подпись в отдельном файле'
            browser.click_on_element_by(
                By.XPATH, "//*[contains(text(), 'Подпись в отдельном файле')]")
        # Отправляем файл data_for_signature.txt для подписи
        data_path = abspath("data/data_for_signature.txt")
        browser.get_element_by(
            By.CSS_SELECTOR, "input[type=file]", is_visible=False).send_keys(data_path)
        # Нажимаем кнопку 'Подписать'
        browser.click_on_element_by(
            By.XPATH, "//*[contains(text(), 'Подписать')]")
        # Проверяем создание подписи
        browser.get_element_by(
            By.XPATH, "//*[contains(text(), 'data_for_signature.txt')]")
