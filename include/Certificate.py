...
class Certificate:
    def __init__(self, browser: Browser, key_id: str, alrorithm: str, is_test_mode: bool = True):
        ...
        if (is_test_mode):
            ...
            browser.click_on_element_by(By.XPATH, f"//*[contains(text(), '{key_id}')]/../../../..//button[contains(text(), 'Добавить тестовый сертификат')]")
            ...
            # TODO:  Убрать лишние условия в проверках существования элементов 
            try:
                element_text = browser.get_element_by(By.XPATH, f"//*[contains(text(), '{key_id}')]/../../..//a[contains(text(), 'Показать всю информацию')]")
            ...
            cert_value = self.__get_cryptopro_certificate_value(browser, self.cert_req).decode('utf-8')
            ...
            browser.click_on_element_by(By.XPATH, f"//*[contains(text(), '{key_id}')]/../../../..//button[contains(text(), 'Добавить сертификат')]")
            # Выбираем вариант 'Данные из буфера обмена'
            browser.click_on_element_by(By.XPATH, "//*[contains(text(), 'Данные из буфера обмена')]")
            # Вводим данные сертификата
            # browser.click_on_element_by(By.XPATH, f"//*[@class = 'p-1']").send_keys(cert_value)
            logger.debug(f"type cert_value {type(cert_value)}")
            browser.get_element_by(By.XPATH, f"//*[@class = 'p-1']").send_keys(cert_value)
            # Нажимаем кнопку 'Связать'
            browser.click_on_element_by(By.XPATH, "//*[contains(text(), 'Связать')]")

            # Проверяем создание связки
            try:
                element_text = browser.get_element_by(By.XPATH, "//*[contains(text(), 'Сертификат импортирован!')]")
                logger.debug("Найдена надпись 'Сертификат импортирован!'")
                # if (element_text.text == "Запрос создан и подписан!"):
                logger.debug(f"Сертификат для ключа {key_id} успешно сгенерирован")
                self.is_created = True
            except TimeoutException:
    ...
    def delete(self, browser: Browser) -> None:
        if (self.is_test_mode): self.key.delete(browser)
        else: self.cert_req.delete(browser)
        self.is_created = False
    ...
    def __get_cryptopro_certificate_value(self, browser: Browser, certificate_request: str) -> str:
        ...
        # time.sleep(3)
        browser.get_element_by(By.CSS_SELECTOR, "#locTaRequest").send_keys(certificate_request.value)
        # time.sleep(3)
